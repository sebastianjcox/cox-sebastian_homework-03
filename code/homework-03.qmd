...
**b) Visual Clarity** 
Table 1 is exceptionally clear: the variables are self-explanatory, 
the F-statistic and associated p-value for each is given, justifying the
inclusion of the variable in the model. Table 2 contains much more information 
and is less clear on its own. It includes the same metrics as Table 1 for many 
more variables and separates those included and not included in the final model.
For those included, it provides model estimates and standard errors. However, 
some of the variable names are unclear and require an additional table that 
provides their meanings.

**c) Aesthetic Clarity** 
Table 1 is clean, uncluttered, and direct. Table 2 contains more information so 
it inherently takes longer to analyze, but it could also be improved with 
stylistic choices. Displaying the "included" or "excluded" text in bold would 
make the separation between variable categories more apparent, and the variables
could have been sorted by descending p-value to indicate which were most 
significant. Furthermore, the values in the 'Estimate' column are quite far 
from the variable names associated with them, so it is difficult to immediately
make the connection between the numbers and the associated variable.
